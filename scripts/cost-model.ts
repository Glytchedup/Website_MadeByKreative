...
type Row = {
  ...
  twine: boolean; htv: boolean; laborMin: number; current: number;
};
const ROWS: Row[] = [
  // Shabby rag garland (twine base, no tape)
  { type: "Rag garland", size: "3 ft", fabricYd: 0.40, tapeYd: 0, twine: true, htv: false, laborMin: 25, current: 12 },
  { type: "Rag garland", size: "4 ft", fabricYd: 0.55, tapeYd: 0, twine: true, htv: false, laborMin: 32, current: 15 },
  { type: "Rag garland", size: "5 ft", fabricYd: 0.70, tapeYd: 0, twine: true, htv: false, laborMin: 42, current: 17.5 },
  { type: "Rag garland", size: "6 ft", fabricYd: 0.85, tapeYd: 0, twine: true, htv: false, laborMin: 52, current: 20 },
  // Regular pennant bunting (7 x 5.5), double-sided
  { type: "Regular pennant", size: "48 in (7)", fabricYd: 0.46, tapeYd: 1.5, twine: false, htv: false, laborMin: 55, current: 16 },
  { type: "Regular pennant", size: "67 in (10)", fabricYd: 0.66, tapeYd: 2.0, twine: false, htv: false, laborMin: 75, current: 20 },
  // Medium pennant (5.5 x 5)
  { type: "Medium pennant", size: "48 in (7)", fabricYd: 0.35, tapeYd: 1.5, twine: false, htv: false, laborMin: 45, current: 14 },
  { type: "Medium pennant", size: "67 in (11)", fabricYd: 0.54, tapeYd: 2.0, twine: false, htv: false, laborMin: 62, current: 18 },
  // Mini pennant (3.5 x 3)
  { type: "Mini pennant", size: "48 in (13)", fabricYd: 0.23, tapeYd: 1.5, twine: false, htv: false, laborMin: 50, current: 10 },
  { type: "Mini pennant", size: "72 in (19)", fabricYd: 0.34, tapeYd: 2.2, twine: false, htv: false, laborMin: 70, current: 15 },
  // Hand-tied bow garland
  { type: "Bow garland", size: "36 in (5)", fabricYd: 0.35, tapeYd: 0, twine: true, htv: false, laborMin: 40, current: 20 },
  { type: "Bow garland", size: "70 in (9)", fabricYd: 0.60, tapeYd: 0, twine: true, htv: false, laborMin: 62, current: 25 },
  // Birthday bunting (regular pennant + HTV lettering, single 73" size)
  { type: "Birthday pennant", size: "73 in (11)", fabricYd: 0.70, tapeYd: 2.2, twine: false, htv: true, laborMin: 70, current: 15 },
];
...
function impliedWage(row: Row): number {
  const mat = materials(row);
  const fee = ETSY_PCT * row.current + ETSY_FIXED;
  const profitBeforeLabor = row.current - mat - fee;
  return profitBeforeLabor / (row.laborMin / 60);
}

const header = [
  "Type", "Size", "Current $", "Fabric yds", "Materials $", "Labor min",
  "Etsy fee @current $", "Implied wage now $/hr",
  ...WAGES.map((w) => `Price to earn $${w}/hr`),
  `Fair-wage price ($${RECOMMEND_WAGE}/hr, rounded)`,
];
const lines = [header.join(",")];

for (const row of ROWS) {
  const rec = tidy(priceForWage(row, RECOMMEND_WAGE));
  lines.push(
    [
      row.type,
      row.size,
...
const avgImplied =
  ROWS.reduce((s, r) => s + impliedWage(r), 0) / ROWS.length;
console.error(`Avg implied wage at current prices: $${avgImplied.toFixed(2)}/hr`);
